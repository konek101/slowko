...
let _resvgClsPromise: Promise<any> | null = null;
async function getResvg() {
  if (_resvgClsPromise) return _resvgClsPromise;
  _resvgClsPromise = import("@resvg/resvg-js").then((m: any) => m.Resvg);
...
const _headerCache = new Map<string, any>(); // key: JSON.stringify({w,text,fontPath,bg}) -> Jimp
const _boardCache = new Map<string, any>(); // currently unused but kept for parity
const _avatarCache = new Map<string, any>(); // key: `${src}|${mtime}|${diameter}` or `placeholder:${diameter}` -> Jimp
const _panelCache = new Map<string, any>(); // key: JSON.stringify({w,h,fill,stroke,sw,rad}) -> Jimp
const CACHE_LIMIT = { header: 32, board: 128, avatar: 64, panel: 32 } as const;
...
let _fontDataUrl: string | null = null; // 'data:font/ttf;base64,...'
let _warmupPromise: Promise<void> | null = null;
let _jimpPromise: Promise<any> | null = null; // lazy-loaded Jimp
async function getJimp() {
  if (_jimpPromise) return _jimpPromise;
  _jimpPromise = import("jimp").then((m: any) => m.default || m);
  return _jimpPromise;
}

async function getFontDataUrl(fontPath?: string | null) {
...
async function renderHeaderToJimp(
  width: number,
  text: string,
  ...
  if (!fontPath) fontPath = getDefaultFontPath();
  try {
    if (fontPath) {
      const svg = `<?xml version="1.0" encoding="UTF-8"?>
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="64">
  <defs>
    <style>
      @font-face { font-family: 'ShareHeader'; src: url('file://${(
        fontPath as string
      ).replace(/\\/g, "/")}'); font-weight: normal; font-style: normal; }
    </style>
  </defs>
  <rect width="100%" height="100%" fill="${background}"/>
  <text x="50%" y="50%" fill="#ffffff" font-family="ShareHeader" font-size="32" dominant-baseline="middle" text-anchor="middle">${escapeXml(
    text
  )}</text>
</svg>`;
      const Resvg = await getResvg();
  ...
  if (/ł|Ł/.test(text)) {
    console.warn(
      "ł character requires a custom TTF font. Provide options.fontPath to avoid ?"
    );
  }
...
let _defaultFontPath: string | null | undefined = undefined; // undefined => not resolved yet; null => tried but none found
...
function getDefaultFontPath(): string | null {
  if (_defaultFontPath !== undefined) return _defaultFontPath as string | null; // cached
  ...
  console.warn(
    "No default TTF font found. Polish characters may not render. Provide options.fontPath."
  );
  ...
  for (let iy = y; iy < y + h; iy++) {
    for (let ix = x; ix < x + w; ix++) {
      ...
      const inTopRight =
        ix >= x + w - radius &&
        iy < y + radius &&
        rx2 * rx2 + ry * ry > radius * radius;
      const inBottomLeft =
        ix < x + radius &&
        iy >= y + h - radius &&
        rx * rx + ry2 * ry2 > radius * radius;
      const inBottomRight =
        ix >= x + w - radius &&
        iy >= y + h - radius &&
        rx2 * rx2 + ry2 * ry2 > radius * radius;
...
export async function generateBoardPng(
  ...
  const png = new PNG({ width, height } as any);
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      const { fill, border } = tileStyle(state[r][c]) as any;
      drawRect(png, x, y, tileSize, tileSize, fill as string);
      if (border) {
  ...
  return await new Promise<Buffer>((resolve, reject) => {
    ...
    (png as any)
      ...
      .on("end", () => {
        // Cast to satisfy TS types that expect Uint8Array[]
...
export async function writeBoardPng(
  state: string[][],
  filePath: string,
  opts?: { tileSize?: number; gap?: number; pad?: number; rounded?: boolean }
) {
  const buf = await generateBoardPng(state, opts);
  await fs.writeFile(filePath, new Uint8Array(buf as any));
  return filePath;
}

type PlayerIn = { state: string[][]; avatar?: string | null };

export async function generateShareImageFast(
  stateOrPlayers: string[][] | PlayerIn[],
  wordleNumber: number | string,
  options: {
    avatar?: string | null;
  ...
  function boardDims(rows: number, cols: number) {
    return {
  ...
  const specs = players.map((p) => {
    ...
    if (players.length === 1) {
      ...
      return { rows, cols, bd, avatarD, layout: "single" as const };
    }
    ...
    return { rows, cols, bd, avatarD, cardW, cardH, layout: "multi" as const };
  });

  const avatarRefs = players.map((p) => {
    if (!p.avatar) return null;
    return "file://" + (p.avatar as string).replace(/\\/g, "/");
  });

  function renderBoardRects(
    x0: number,
    y0: number,
    spec: any,
    playerIndex: number
  ...
  if (players.length === 1) {
    ...
    const avY =
      panelY +
      panelPad +
      Math.round((Math.max(s.avatarD, s.bd.h) - s.avatarD) / 2);
    const boardX = avX + s.avatarD + innerSpacing;
    const boardY =
      panelY +
      panelPad +
      Math.round((Math.max(s.avatarD, s.bd.h) - s.bd.h) / 2);
    const avId = `av0`;
    const avImg = avatarRefs[0];
    svgCards += `
  <g>
    <rect x="${panelX}" y="${panelY}" width="${panelW}" height="${panelH}" rx="16" ry="16" fill="${panelFill}" stroke="${panelStroke}" />
    <defs><clipPath id="${avId}"><circle cx="${avX + s.avatarD / 2}" cy="${
      avY + s.avatarD / 2
    }" r="${s.avatarD / 2}"/></clipPath></defs>
    ${
      avImg
        ? `<image href="${avImg}" x="${avX}" y="${avY}" width="${s.avatarD}" height="${s.avatarD}" clip-path="url(#${avId})"/>`
        : `<circle cx="${avX + s.avatarD / 2}" cy="${avY + s.avatarD / 2}" r="${
            s.avatarD / 2
          }" fill="#218ca0"/>`
    }
    ...
    const cardsW = specs.reduce(
      (sum, s, i) => sum + (s as any).cardW + (i ? colSpacing : 0),
      0
    );
    contentW = cardsW;
    contentH = Math.max(...specs.map((s: any) => s.cardH));
    let cursorX = 0;
    const cardsTop = 0;
  ...
  const fontDataUrl = await getFontDataUrl(fontPath);
  const svg = `<?xml version="1.0" encoding="UTF-8"?>
<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
  <defs>
    <style>
      ${
        fontDataUrl
          ? `@font-face { font-family: 'ShareHeader'; src: url('${fontDataUrl}'); }`
          : ""
      }
  ...
  <text x="50%" y="${Math.round(
    headerH / 2
  )}" fill="#ffffff" font-size="36" dominant-baseline="middle" text-anchor="middle">${escapeXml(
    headerText
  )}</text>
...
export async function generateShareImage(
  ...
  if (players.length === 1) {
    ...
    const panelPad = 24;
    const panelWidth =
      panelPad * 2 + avatarDiameter + innerSpacing + boardWidth;
    ...
    let headerImage: any;
    try {
      headerImage = await renderHeaderToJimp(
        totalWidth,
        headerText,
    ...
    const panelImg = await renderRoundedPanel(
      panelWidth,
      panelHeight,
      panelFill,
      "#2a2a2b",
    ...
    const avatarImg = await renderAvatarCircleToJimp(
      p.avatar || null,
      avatarDiameter
    );
    const avX = panelX + panelPad;
    const avY =
      panelY +
      panelPad +
      Math.round((Math.max(avatarDiameter, boardHeight) - avatarDiameter) / 2);
    image.composite(avatarImg, avX, avY);

    const boardBuf = await generateBoardPng(p.state, {
      tileSize,
      gap,
      pad: padBoard,
      rounded: false,
    ...
    const boardY =
      panelY +
      panelPad +
      Math.round((Math.max(avatarDiameter, boardHeight) - boardHeight) / 2);
  ...
  const cardSpecs = await Promise.all(
    players.map(async (p) => {
      const rows = p.state.length;
      const cols = p.state[0]?.length || 0;
      if (!rows || !cols)
        throw new Error("Each player.state must be a non-empty grid");
      const tileSize = options.tileSize ?? (cols <= 5 ? 32 : 28);
      const gap = options.gap ?? 6;
      const padBoard = options.padBoard ?? 16;
      const boardWidth = padBoard * 2 + cols * tileSize + (cols - 1) * gap;
  ...
  const totalWidth =
    marginX * 2 +
    cardSpecs.reduce(
      (sum, s, i) => sum + s.cardWidth + (i > 0 ? colSpacing : 0),
      0
    );
  ...
  try {
    ...
    console.warn(
      "Header render failed, falling back to bitmap font:",
      e?.message
    );
    const font = await Jimp.loadFont(Jimp.FONT_SANS_32_WHITE);
    headerImage = new Jimp(totalWidth, 64, COLORS.background);
...
function normalizePlayers(
  ...
  if (
    Array.isArray(stateOrPlayers) &&
    Array.isArray((stateOrPlayers as any)[0]) &&
    typeof (stateOrPlayers as any)[0][0] === "string"
  ) {
    return [
      { state: stateOrPlayers as string[][], avatar: options?.avatar ?? null },
    ];
  }
  if (
    Array.isArray(stateOrPlayers) &&
    (stateOrPlayers as any).length &&
    typeof (stateOrPlayers as any)[0] === "object" &&
    Array.isArray((stateOrPlayers as any)[0].state)
  ) {
    return stateOrPlayers as PlayerIn[];
  }
  return [] as PlayerIn[];
}
...
async function renderRoundedPanel(
  width: number,
  height: number,
  fill: string,
  stroke: string,
...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}">
  <rect x="0.5" y="0.5" width="${width - 1}" height="${
    height - 1
  }" rx="${radius}" ry="${radius}" fill="${fill}" stroke="${stroke}" stroke-width="${strokeWidth}" />
</svg>`;
  const Resvg = await getResvg();
  const resvg = new Resvg(svg, { fitTo: { mode: "width", value: width } });
...
async function renderAvatarCircleToJimp(
  ...
  try {
    ...
    if (srcPath) {
      try {
        mtime = fsSync.statSync(srcPath).mtimeMs || 0;
      } catch {}
    }
...
<svg xmlns="http://www.w3.org/2000/svg" width="${diameter}" height="${diameter}">
  <defs>
    <clipPath id="clip"><circle cx="${diameter / 2}" cy="${diameter / 2}" r="${
        diameter / 2
      }"/></clipPath>
  </defs>
  <image href="${b64}" x="0" y="0" width="${diameter}" height="${diameter}" clip-path="url(#clip)"/>
      ...
      const resvg = new Resvg(svg, {
        fitTo: { mode: "width", value: diameter },
      });
...
function buildWindowsScreenClipPath(filenameGuid: string) {
  const localAppData =
...
export async function writeShareImage(
  stateOrPlayers: string[][] | PlayerIn[],
  wordleNumber: number | string,
  options: {
    outputPath?: string;
  ...
  const players = normalizePlayers(stateOrPlayers, options);
  const buf = await generateShareImageFast(
    stateOrPlayers,
    wordleNumber,
    options
  );

  let outPath = options.outputPath as string | undefined;
  if (!outPath && players.length === 1) {
    ...
    const built = buildWindowsScreenClipPath(guid as string);
    if (built) {
  ...
  console.log("[share] wrote", outPath, "bytes=", (buf as any).length);
  return outPath;
}

export async function warmShareRenderer(
  options: { fontPath?: string | null } = {}
) {
  try {
...
<svg xmlns="http://www.w3.org/2000/svg" width="8" height="8">
  ...
  <text x="4" y="4" font-family="Warm" font-size="6" fill="#fff" dominant-baseline="middle" text-anchor="middle">ł</text>
    ...
    console.warn("warmShareRenderer failed:", e?.message || e);
    return false;
  }
}
