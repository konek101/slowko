...
async function main() {
  ...
  const buf = await generateShareImage(state as any, wordleNumber, { fontPath });
  await writeFile('share-test.png', buf as any);
  console.log('wrote share-test.png', (buf as any).length ?? '');
}

main().catch(e => {
  console.error('Failed to generate share image:', (e as any)?.stack || e);
  process.exit(1);
});
