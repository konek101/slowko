...
async function upsertEntryPoint() {
  ...
  default_member_permissions: null,
    // Critical for Activity Entry Point
  ...
  if (GUILD_ID) {
    ...
    const gcmds = await listGuild.json();
    ...
    if (!existingG) {
      ...
      console.log('[entrypoint] Created GUILD command', created.id, 'name', created.name);
    } else {
      ...
      console.log('[entrypoint] Updated GUILD command', updated.id, 'name', updated.name);
    }
  }
  ...
  const commands = await listResp.json();
  ...
  if (!existing) {
    ...
    console.log('[entrypoint] Created GLOBAL command', created.id, 'name', created.name);
  } else {
    ...
    console.log('[entrypoint] Updated GLOBAL command', updated.id, 'name', updated.name);
  }
}

upsertEntryPoint().catch(err => {
  console.error('[entrypoint] Error:', err?.message || err);
  process.exit(1);
});
